package monica.container.server;
...
import monica.service.config.ConfigServiceInit;
...
public class ServerContainer {
	...
	public  void start() throws Exception{
		...
		// CountDownLatch instance will wait the countDown number which is specified here.		
		...
		String className = protocolString.substring(0, 1).toUpperCase()+protocolString.substring(1,protocolString.length())+"Server";
	    ...
	    for(;;){
	    	// the Server  isStrated filed is visible to multi-thread by volatile modifier
	    	if(server.isStarted()){
	    ...
	    new RegistryCentre()
	    	.setUri(createUri(server))
	    	.setType(RegistryType.SERVER)
	        .start();
